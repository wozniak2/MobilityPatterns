...
competitive_by_muni <- table(competitive$municipality_short) %>% as.data.frame() %>% arrange(-Freq)
competitive_by_muni

#Take a look at the least competitive itineraries (to be improved)
improvement <- itineraries %>% filter(time_ratio > 2.5)
improvement_by_muni <- table(improvement$municipality_short) %>% as.data.frame() %>% arrange(-Freq)
improvement_by_muni
